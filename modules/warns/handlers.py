...
from utils.permissions import can_moderate, get_user_role, is_target_protected
...
async def _authorize_target(client: Client, message: Message) -> User:
    """Resolve a warn target and verify the invoker outranks them.
    ...
    Raises:
        ...
        PermissionDeniedError: If the target is protected, is the invoker
            themselves, or is not outranked by the invoker.
    """
...
async def _handle_warn(client: Client, message: Message) -> None:
    ...
    reason = extract_reason(message, offset=_target_offset(message)) or "No reason given"
